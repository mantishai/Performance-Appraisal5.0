import { Toast, Skeleton, EventBus } from '../utils.js';
...
// 监听员工更新事件，更新岗位信息
EventBus.on('employee:updated', async (employee) => {
    console.log('[Dashboard] 员工信息更新:', employee);
    // 如果更新的是当前用户，可以刷新数据
    if (state.currentUser && (employee.employee_no === state.currentUser.employee_no || employee.id === state.currentUser.id)) {
